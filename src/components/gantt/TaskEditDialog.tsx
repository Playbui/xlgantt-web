...
export function TaskEditDialog({ taskId, open, onClose }: TaskEditDialogProps) {
  ...
  const handleOpenWorkspaceList = () => {
    if (linkedWorkspaceItems[0]) selectWorkspaceItem(linkedWorkspaceItems[0].id)
    setActiveView('workspace')
    onClose()
  }
            ...
            <div className="space-y-4">
              ...
              <Section icon={NotebookText} title="연결된 업무노트" count={linkedWorkspaceItems.length}>
                {linkedWorkspaceItems.length > 0 ? (
                  <div className="space-y-2">
                    <div className="max-h-40 overflow-y-auto rounded-md border border-slate-200 bg-slate-50/70">
                      {linkedWorkspaceItems.map((item) => (
                        <button
                          key={item.id}
                          type="button"
                          onClick={() => handleOpenWorkspaceItem(item.id)}
                          className="flex w-full items-center gap-2 border-b border-slate-200 bg-background/80 px-2.5 py-2 text-left text-xs last:border-b-0 hover:bg-primary/5"
                        >
                          <NotebookText className="h-3.5 w-3.5 flex-shrink-0 text-primary/70" />
                          <div className="min-w-0 flex-1">
                            <div className="flex min-w-0 items-center gap-1.5">
                              <span className="truncate font-semibold text-foreground">{item.title || '제목 없는 문서'}</span>
                              <Badge variant="outline" className="h-4 px-1 text-[10px]">{WORKSPACE_STATUS_LABELS[item.status]}</Badge>
                            </div>
                            <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-muted-foreground">
                              ...
                              {item.access_mode !== 'project' && (
                                <>
                                  <span>·</span>
                                  <span>{WORKSPACE_ACCESS_LABELS[item.access_mode]}</span>
                                </>
                              )}
                            </div>
                          </div>
                    ...
                    <Button variant="outline" size="sm" className="h-7 w-full text-xs" onClick={handleOpenWorkspaceList}>
                      업무노트에서 전체 보기
                    </Button>
                  </div>
                ) : (
                  <button
                    ...
                    className="w-full rounded-md border border-dashed border-slate-300 bg-slate-50/70 px-3 py-3 text-center text-xs text-muted-foreground hover:border-primary/40 hover:text-primary"
                  >
                    연결된 업무노트가 없습니다. 업무노트에서 WBS와 문서를 연결하세요.
                  </button>
                )}
              </Section>
            </div>
